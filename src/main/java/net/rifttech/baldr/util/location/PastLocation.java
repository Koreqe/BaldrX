...
public class PastLocation {
    ...
    public List<CustomLocation> getEstimatedLocation(long time, long timestamp, long delta) {
        List<CustomLocation> locs = new ArrayList<>();

        previousLocations.stream()
                .sorted(Comparator.comparingLong(loc -> Math.abs(loc.getTimeStamp() - (timestamp - time))))
                .filter(loc -> Math.abs(loc.getTimeStamp() - (timestamp - time)) < delta)
                .forEach(locs::add);
        return locs;
    }
